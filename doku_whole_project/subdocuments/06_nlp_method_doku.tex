...
ITEM : Cookies
        \end{lstlisting}
        Every symbol is nonterminal, except for symbols defined in the \{.voca} file. Terminals are defined as a word category. The category is defined through \newline \lstinline{% <<name>>}
        In the \{.voca} file the word and the phonetic representation are separated by a space.
        \begin{lstlisting}
% and
and ae n d
% move
...
Oreos ao r iy ow s
        ...
        Grammars have to be compiled to run. To compile run \{rosrun tmc\_julius mkdfa.pl <<grammar\_name>>} e.g. \{rosrun tmc\_julius mkdfa.pl grammar}. Both the \{.grammar} and \{.voca} file have to be named identical. E.g. \{grammar.grammar} and \{grammar.voca}. The compiler creates the \{.dfa}, \{dict} and \{.term} files.	
    ...
    \subsection{Configuration files}
        The \{julius\_conf} folder contains one \{.jconf} file. The context of this file has been taken from the \{tmc\_julius} package. This part of the package has been created to possibly handle configurations for different computers and if possible experiment with the configuration to increase recognition accuracy.
    \subsection{Launchfiles}
        The \{suturo\_julius} package provides four different launch files.\\
        \begin{itemize}
...
\chapterauthor{Paul Schnipper}
    \subsection{textparser.py}
        ...
        \subsubsection{callback(data)}
            The \{callback} function is called with the data published by Julius. Julius publishes its output in the \{tmc\_rosjulius\_msgs.msg.RecognitionResult} message type. The \{callback} function calls the \{find\_first} function to get a String representing the recognized sentence. It is then checked if the String is a static command. To do that the \{is\_static} command is called. If the recognized sentence is not a static command the sentence is published on the \{/to\_slingparser} topic to be analysed by SLING.
    
        \subsubsection{find\_first(data)}
            The \{find\_first} function takes the data published by Julius and checks weather the recognition confidence is high enough. If so it returns the recognized sentence as String, if the confidence is not high enough the function returns an empty string.\\
    
        \subsubsection{is\_static(sentence)}
            The \{is\_static} command calls the \{is\_stop}, \{is\_start} and \{is\_continue} functions. If the sentence is one of these hard command types the function publishes the command type on the \{'hard\_commands'} topic.
    
        \subsubsection{is\_stop, is\_start and is\_continue}
            These functions check weather a string is a hard command of the type \{STOP, START or CONTINUE} respectively. To perform this check the \{is\_in\_file} function is called. The text files containing the hard commands can be found in the \{commands} folder.
...
\{MovementOnly\\
        ...
        \subsubsection{About TinyRPC}
             ...
             To solve this problem, the \{slingparserRosInterface.py} starts \{slingparser.py3} as a subprocess. The slingparser.py3 upon startup initializes the tinyRPC server and provides the \{semantic\_parse()} function.
        \subsubsection[slingparserros]{slingparserRosInterface.py}
            The \{slingparserRosInterface.py} subscribes to a topic on which \{String} messages are published. It then calls the the semantic parse function provided by the \{slingparser.py} script. This functions returns the annotated semantic frames in a python dictionary. This dictionary is then translated into a \{MeaningRepresentation} message. This message definition contains an array of \{KeyValuePair} messages. Each of these messages contains the frame as a key and the contents of the frame as the associated value.
        \subsubsection{slingparser.py3}
            The slingparser.py3 script wraps the Sling Semantic Parser. It currently loads a hard coded \{flow} file.
            The \{semantic\_parse} function actually calls the sling parser and takes its output and writes it into a dictionary to return.
    
\section{The suturo\_nlg Package}
\chapterauthor{Paul Schnipper}
    The \{suturo\_nlg} package handles speech generation. But also contains the \{knowledge\_interface} node, designed to answer simple questions by querying KnowRob through ROSProlog.
    \subsection{knowledge\_interface.py}
        ...
        The \{knowledge\_interface} subscribes to a topic using the \{MeaningRepresentation} message. (See \hyperref[slingparserros]{slingparserRosInterface}) When there is a message published on this topic the \{callback} function is called.
        \subsubsection{callback}
            The callback function takes the \{MeaningRepresentation} message and converts it into a dictionary.
...
\section{The suturo\_speechparser Package}
    ...
    \begin{itemize}
        ...
        \item \{speechparser.launch}
            \subitem Placeholder for a later system similar to \{qa\_system.launch}. This is supposed to be used for complex commands given to the robot.  
